...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerImg = await loadImageAsync("assets/MiniHalberdMan.png");
  const boarImg = await loadImageAsync("assets/MiniSpearMan.png");
  const leafImg = await loadImageAsync("assets/drculacat.png");
...
function defaultPlayerAnis() {
  return {
    ...
    death: { row: 5, frames: 5, frameDelay: 25 },
    hurtPose: { row: 4, frames: 3, frameDelay: Infinity },
...
function defaultBoarAnis() {
  return {
    ...
    throwPose: { row: 3, frames: 7, frameDelay: Infinity, frame: 0 },
    death: { row: 5, frames: 4, frameDelay: 10 },
  };
}
...
async function loadBackgrounds(levelPkg) {
  ...
  return {
    bgFar: await loadImageAsync("assets/GandalfHardcore Background layers_layer 5.png"),
    bgMid: await loadImageAsync("assets/GandalfHardcore Background layers_layer 2.png"),
    bgFore: await loadImageAsync("assets/GandalfHardcore Background layers_layer 1.png"),
  };
}
